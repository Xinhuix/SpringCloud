 ...
 */
@Configuration
@MapperScan(basePackages = { "com.visionvera.dao.operation" }, sqlSessionFactoryRef = "operationSqlSessionFactory")
public class ResourceDataSourceConfig extends DataSourceBaseConfig {
	/** XML配置文件存放路径 */
	private final String MAPPER_LOCATION = "classpath:com/visionvera/dao/operation/*.xml";
	 ...
	 * 配置事务
	 */
	@Bean(value = "transactionManager_operation")
	@Primary//该Bean会被优先考虑。在多数据源的环境中必须且只加在其中一个配置里
	 ...
	 * @throws Exception 
	 */
	@Bean(value = "operationSqlSessionFactory")
	@Primary//该Bean会被优先考虑。在多数据源的环境中必须且只加在其中一个配置里
	public SqlSessionFactory getSqlSessionFactory(@Qualifier("operationDataSource") DataSource dataSource) throws Exception {
