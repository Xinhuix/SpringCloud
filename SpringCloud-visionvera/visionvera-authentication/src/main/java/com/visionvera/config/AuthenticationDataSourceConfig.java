 ...
 */
@Configuration
@MapperScan(basePackages = { "com.visionvera.dao.authentication" }, sqlSessionFactoryRef = "authenticationSqlSessionFactory")
public class AuthenticationDataSourceConfig extends DataSourceBaseConfig {
	/** XML配置文件存放路径 */
	private final String MAPPER_LOCATION = "classpath:com/visionvera/dao/authentication/*.xml";
	 ...
	 * 配置事务
	 */
	@Bean(value = "transactionManager_authentication")
	@Primary//该Bean会被优先考虑。在多数据源的环境中必须且只加在其中一个配置里
	 ...
	 * @throws Exception 
	 */
	@Bean(value = "authenticationSqlSessionFactory")
	@Primary//该Bean会被优先考虑。在多数据源的环境中必须且只加在其中一个配置里
	public SqlSessionFactory getSqlSessionFactory(@Qualifier("authenticationDataSource") DataSource dataSource) throws Exception {
