 ...
 */
@Configuration
@MapperScan(basePackages = { "com.visionvera.dao.ywcore" }, sqlSessionFactoryRef = "ywcoreSqlSessionFactory")
public class YwcoreDataSourceConfig extends DataSourceBaseConfig {
	/** XML配置文件存放路径 */
	private final String MAPPER_LOCATION = "classpath:com/visionvera/dao/ywcore/*.xml";
	
	/** 别名包 */
	private final String TYPE_ALIASES_PACKAGE = "com.visionvera.bean";

	@Value("${ywcore.datasource.url}")
	private String url;

	@Value("${ywcore.datasource.username}")
	private String username;

	@Value("${ywcore.datasource.password}")
	private String password;

	@Value("${ywcore.datasource.driverClassName}")
...
//	@Primary//该Bean会被优先考虑。在多数据源的环境中必须且只加在其中一个配置里
	 ...
	 * 配置事务
	 */
	@Bean(value = "transactionManager_ywcore")
//	@Primary//该Bean会被优先考虑。在多数据源的环境中必须且只加在其中一个配置里
	 ...
	 * @throws Exception 
	 */
	@Bean(value = "ywcoreSqlSessionFactory")
//	@Primary//该Bean会被优先考虑。在多数据源的环境中必须且只加在其中一个配置里
	public SqlSessionFactory getSqlSessionFactory(@Qualifier("ywcoreDataSource") DataSource dataSource) throws Exception {
