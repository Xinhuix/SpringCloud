 ...
 */
@Configuration
@MapperScan(basePackages = { "com.visionvera.dao.perception" }, sqlSessionFactoryRef = "perceptionSqlSessionFactory")
public class PerceptionDataSourceConfig extends DataSourceBaseConfig {
	/** XML配置文件存放路径 */
	private final String MAPPER_LOCATION = "classpath:com/visionvera/dao/perception/*.xml";
	 ...
	 * @return
	 */
	@Bean(name = "perceptionDataSource")
	 ...
	 * 配置事务
	 */
	@Bean(value = "transactionManager_perception")
	@Primary//该Bean会被优先考虑。在多数据源的环境中必须且只加在其中一个配置里
	 ...
	 * @throws Exception 
	 */
	@Bean(value = "perceptionSqlSessionFactory")
	@Primary//该Bean会被优先考虑。在多数据源的环境中必须且只加在其中一个配置里
	public SqlSessionFactory getSqlSessionFactory(@Qualifier("perceptionDataSource") DataSource dataSource) throws Exception {
