...
public interface SlweomsDao {
    ...
    List<RegionVO> selectRegion(Slweoms slweoms);
     ...
     * @return
     */
    List<ServerHardwareVO> selectRegionServerList(Slweoms slweoms);

    /**
     * 查询硬件历史信息
     *
     * @param time         15分钟 30 分钟 一小时 以及每分钟  对应参数  15,30,60,1
     * @param serverUnique 服务器唯一标识
     * @param now          当日时间
     * @return
     */
    @MapKey("time")
    Map<String, Map<String, Object>> selectServerHardwareHistory(@Param("time") Integer time, @Param("serverUnique") String serverUnique, @Param("now") Date now);
     ...
     * @return
     */
    Map<String, Object> selectThreshold(@Param("serverUnique") String serverUnique);

    /**
     * 查询所有服务器
     * @return
     */
    List<String> selectServerBasics();

    List<Map<String, Object>> selectServerHardwareHistoryMinute(@Param("serverUnique") String serverUnique,@Param("startTime") String startTime,@Param("endTime") Date endTime);
}
